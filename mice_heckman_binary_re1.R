...
mice.impute.2l.heckman1step_re_ghq <- function(y, ry, x, type, QP=rep(10,2),draw=T,excl="",...) { 
  ...
  start2 <- c(0,0)
  modes <- t(sapply(1:length(table(data$group)), function(x) { # Funktion wird auf alle Cluster angewandt 
...
mice.impute.2l.heckman1step_re_ghq <- compiler::cmpfun(mice.impute.2l.heckman1step_re_ghq)
